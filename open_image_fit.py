...
if mag == True:
        ...
        # filename_AIA = glob.glob('AIA/aia.lev1.1600A_2021-05-22T06 17 02.13Z.image_lev1.fits')
        filename_AIA = glob.glob('AIA/aia.lev1.1600A_2021-05-22T06 16 14.12Z.image_lev1.fits')
...
for i in cadres:
        ...
        if mag == True:
                ...
                ax.text(0.5,1.05,f"HMI, time 06:18:00 UT", size = size *1.3, weight = "bold", ha="center", transform=ax.transAxes)
                ax.tick_params(axis='both', labelsize = size)
                plt.imshow(imgAIA / np.max(imgAIA) * 100, cmap = 'seismic', origin = "lower", extent = extent_AIA, animated = True, norm = colors.Normalize(vmin=-15, vmax=15))
                # plt.colorbar()
                plt.contour(imgI, levels = levelsI, colors = [(0, i/255, 0) for i in np.linspace(75, 255, len(levelsI))], origin = "lower", extent = extentI, linewidths = 3)
                ...
                ax.text(0.5,1.05,f"AIA 1600 $\AA$ and SRH I, time 06:16:14 UT", size = size *1.3, weight = "bold", ha="center", transform=ax.transAxes)
                ax.tick_params(axis='both', labelsize = size)
                plt.imshow(imgAIA / np.max(imgAIA) * 100, cmap = 'hot', origin = "lower", extent = extent_AIA, norm = colors.PowerNorm(gamma = 0.5))
                # plt.colorbar()
                plt.contour(imgI, levels = levelsI, colors = [(0, 0, i/255) for i in np.linspace(75, 255, len(levelsI))], origin = "lower", extent = extentI, linewidths = 3)
                plt.xlabel("Helioprojective Longtitude, [arcsec]", size = size * 1.1, weight = "bold")
                plt.ylabel("Helioprojective Latitude, [arcsec]", size = size * 1.1, weight = "bold")
                plt.axis([-550, -250, 250, 500])
                # plt.axis([-960, -300, 200, 500])
